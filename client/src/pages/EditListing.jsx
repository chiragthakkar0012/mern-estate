...
{   
    ...
    const [listingData,setListingData]=useState({imageUrls:[...state.imageUrls],
       type:state.type,
       parking:state.parking||false,
       furnished:state.furnished||false,
       offer:state.offer||false,
       userRef:state.useRef,
       discountPrice:state.discountPrice||false,
   ...
   else 
   {
    ...
    const handleFileChange = (e) => {
      ...
      {  
        ...
        try { setLoading(true);
            const res=await fetch(`/api/listing/update/${state._id}`,{
                method:'POST',
                headers:{
   ...
   <main className='p-3 max-w-4xl mx-auto'>
    ...
    <form className='flex flex-col sm:flex-row gap-4' onSubmit={handleFormSubmit} onFocus={handleFocus}>
        <div className='flex flex-col gap-4 flex-1'>
            <input type="text" placeholder='Name'  value={listingData.name} className='border p-3 rounded-lg' id='name' maxLength='62' minLength='10' required onChange={handleFormChange}/>
            <textarea type="text" placeholder='Description' value={listingData.description} className='border p-3 rounded-lg' id='description' required onChange={handleFormChange}/>
            <input type="text" placeholder='Address' className='border p-3 rounded-lg' id='address' maxLength='62' minLength='10'  value={listingData.address} required onChange={handleFormChange}/>
            ...
            <div className='flex gap-2"'>
                <input type='checkbox' id='parking' className='w-5 mr-2'  checked={listingData.parking} onChange={handleFormChange}/>
            ...
            <div className='flex gap-2"'>
                <input type='checkbox' id='offer' checked={listingData.offer} className='w-5 mr-2' onChange={handleFormChange}/>
                <span>Offer</span>
            </div>
        </div>
        <div className='flex flex-wrap gap-6'>
            <div className='flex gap-2 items-center'>
                <input type='number'  id='bedRooms'  min='1'  value={listingData.bedRooms} max='10' required className='p-2 border border-gray-300 rounded-lg ' onChange={handleFormChange}/>
                
                <span>Beds</span>
            </div>
            <div className='flex gap-2 items-center'>
                <input type='number'  id='bathRooms'  min='1'  max='10' value={listingData.bathRooms} required className='p-2 border border-gray-300 rounded-lg ' onChange={handleFormChange}/>
                <span>Baths</span>
            </div>
            <div className='flex gap-2 items-center'>
                <input type='number'  id='regularPrice'  min='50'  max='10000000'  value={listingData.regularPrice} required className='p-3 border w-28  border-gray-300 rounded-lg ' onChange={handleFormChange}/>
            ...
            </div>
            {
                ...
                <input type='number'   value={listingData.discountPrice} id='discountPrice'  min='0'  max='100000' required className='p-2 border w-28 border-gray-300 rounded-lg ' onChange={handleFormChange}/>
                <div className='flex flex-col items-center'>
       ...
       <div className='flex flex-col flex-1 gap-4'>
        ...
        <button  disabled={loading||uploading}  className='rounded-lg bg-slate-700 text-white uppercase p-3  hover:opacity-95 disabled:opacity-80'>{loading? 'Editing....':'Edit  Listing'}</button>
        {
